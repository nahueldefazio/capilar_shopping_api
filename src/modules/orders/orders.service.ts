...
export class OrdersService implements OnModuleInit {
  ...
  async create(dto: CreateOrderDto): Promise<Order> {
    const order = await this.dataSource.transaction(async (manager) => {
      ...
      // 5. Build order — every request is a reservation; shipping/payment are coordinated manually afterward
      ...
      const savedShipping = await manager.save(OrderShipping, shippingRecord);
